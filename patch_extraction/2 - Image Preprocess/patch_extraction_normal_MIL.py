...
slide_path_normal = 'E:\\camelyon16\\TrainingData\\slides\\'

anno_path = 'E:/camelyon16/TrainingData/lesion_annotations/'
BASE_TRUTH_DIR = 'E:/camelyon16/TrainingData/Ground_truth/mask/'
all_tumor_patches = "E:\\camelyon16\\dataset_for_training\\10k_dataset\\MIL\\"
...
image_size = 224
Level = 0
df = pd.DataFrame({"slide_name": [], "case_id": [], "label": 1})
...
def random_crop(slide, truth, thresh, crop_size, i, j):
    ...
    # print(rgb_mask)
    rgb_image = rgb_image.resize((image_size, image_size))
...
while i < len(slide_paths):
    ...
    df = df.append({"slide_name": filename, "case_id": filename, "label": 0}, ignore_index=True)
    df.to_csv(all_tumor_patches + "label.csv")
    o= 0
    while o in range(0,50):
        ...
        if (cv2.countNonZero(nr[1]) > crop_size[0] * crop_size[1] * 0.2) and (o <= 300):
            ...
            # imageio.imwrite('E:/camelyon16/dataset_for_training/10k_dataset/imgs/normal/%s_%d_%d.png' % (
            # osp.splitext(osp.basename(slide_paths[i]))[0], nr[2][0], nr[2][1]), img)
            io.imsave(path_filename+ "/" +"0"+str(o) + '.png', img)
            # imageio.imwrite('E:/camelyon16/dataset_for_training/10k_dataset/imgs/normal_mask/%s_%d_%d_mask.png' % (

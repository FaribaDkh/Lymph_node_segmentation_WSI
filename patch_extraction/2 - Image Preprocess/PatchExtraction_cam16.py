...
def None_overlapped_crop_fariba(slide, truth, thresh, crop_size, bbox,num_column,num_rows):
    # width, height = slide.level_dimensions[0]
    dy, dx = crop_size
...
for i in range(len(slide_paths_total)):
    ...
    with openslide.open_slide(slide_paths_total[i]) as slide:
        ...
        with openslide.open_slide(str(truth_slide_path)) as truth:
            ...
            cv.imwrite(path + "/" + filename+".png", thum)
            cv.imwrite(path + "/" + filename + "_binary.png", rgbbinary)
            # define the name of the directory to be created
            # path = "E:/camelyon16/dataset_for_training/test_cam2017/level2/" + filename
            # path_mask = "E:/camelyon16/dataset_for_training/test_cam2017/level2/" + filename
            #
            # try:
            #     os.mkdir(path)
            # except OSError:
            ...
            try:
                ...
                print("Creation of the directory %s failed" % path)
            try:
                os.mkdir(path+"/" + filename+ '/patches')
            except OSError:
                print("Creation of the directory %s failed" % path)
            try:
                os.mkdir(path+"/" + filename + '/masks')
            except OSError:
                print("Creation of the directory %s failed" % path)
            try:
                os.mkdir(path+"/" + filename +'/masks_1')
            except OSError:
                print("Creation of the directory %s failed" % path)
            for j in range(num_rows):
                for k in range(num_column):
                    r = None_overlapped_crop_fariba(slide, truth, thresh,crop_size,bboxt,k,j)
                    # if (cv2.countNonZero(r[2]) > crop_size[0] * crop_size[1] * 0.5):
                    # I used r1 for exracting all patchas with tissue

                    if (cv.countNonZero(r[1]) > crop_size[0] * crop_size[1] * 0.1):
                        ...
                        # if(cv.countNonZero(r[2]) > 2):
                            # io.imsave(patch_path+ 'tumor_mask/%s_%d_%d.png' % (
                            #     osp.splitext(osp.basename(slide_paths[i]))[0], r[3][1], r[3][0]), mask*255)
                            # io.imsave(patch_path+'tumor/%s_%d_%d.png' % (
                            #     osp.splitext(osp.basename(slide_paths[i]))[0], r[3][1], r[3][0]),img)
                            # io.imsave(patch_path+'tumor_mask_1/%s_%d_%d.png' % (
                            #     osp.splitext(osp.basename(slide_paths[i]))[0], r[3][1], r[3][0]), mask)

                        io.imsave(path+"/"+filename +'/masks/%s_%d.png' % (
                            osp.splitext(osp.basename(slide_paths[i]))[0], n), mask*255)
                        io.imsave(path+"/"+filename +'/patches/%s_%d.png' % (
                            osp.splitext(osp.basename(slide_paths[i]))[0], n), img)
                        io.imsave(path+"/"+filename +'/masks_1/%s_%d.png' % (
                            osp.splitext(osp.basename(slide_paths[i]))[0], n), r[1])
                    n = n + 1
                    # print(r[2])

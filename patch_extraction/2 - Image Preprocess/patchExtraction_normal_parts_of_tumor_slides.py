...
def None_overlapped_crop(slide, truth, thresh, crop_size, bboxt):
    # width, height = slide.level_dimensions[0]
    dy, dx = crop_size
    x = np.random.randint(bboxt[0], bboxt[1] - dx + 1)
    y = np.random.randint(bboxt[2], bboxt[3] - dy + 1)
    ...
    with openslide.open_slide(slide_paths_total[i]) as slide:
        ...
        with openslide.open_slide(str(truth_slide_path)) as truth:
            ...
            while o in range(0, 2):
                    ...
                    r = None_overlapped_crop(slide, truth, thresh,crop_size,bboxt)
                    ...
                    if (cv.countNonZero(r[2]) ==0):
                        ...
                        # print(r[2])
                        if(cv.countNonZero(r[1]) > crop_size[0] * crop_size[1] * 0.5):
                            io.imsave("E:\\camelyon16\\50k_dataset\\"+ 'new_normal_tumor_mask/%s_%d_%d.png' % (
                                osp.splitext(osp.basename(slide_paths[i]))[0], r[3][1], r[3][0]), mask*255)
                            io.imsave("E:\\camelyon16\\50k_dataset\\"+'new_normal_tumor/%s_%d_%d.png' % (
                                osp.splitext(osp.basename(slide_paths[i]))[0], r[3][1], r[3][0]),img)
                            io.imsave("E:\\camelyon16\\50k_dataset\\"+'new_normal_tumor_mask_1/%s_%d_%d.png' % (

...
for i in range(len(slide_paths_total)):
    ...
    with openslide.open_slide(slide_paths_total[i]) as slide:
        ...
        cv.imshow("hsv_image",gray_image)
        ret2, rgbbinary = cv.threshold(gray_image,127,255,cv.THRESH_OTSU)
        # rgbbinary = cv.inRange(hsv_image, thresh[0], thresh[1])
        cv.imshow("binary.png", rgbbinary)
        ...
        with openslide.open_slide(str(truth_slide_path)) as truth:
            ...
            path = "E:\\camelyon16\\MIL_Cam17\\"
            path_mask = "E:\\camelyon16\\MIL_Cam17\\"
            ...
            num_columns = int((math.floor(np.max(xxmax) * 256) - math.floor(np.min(xxmin) * 256))/(size*mag_factor))
            ...
            for j in range(num_rows):
                for k in range(num_columns):
                    ...
                    if (cv.countNonZero(r[1]) > crop_size[0] * crop_size[1] * 0.005):
                ...
                #         osp.splitext(osp.basename(slide_paths[i]))[0], r[3][1],r[3][0]), r[0])
                        ...
                        # io.imsave(path+'/imgs/'+str(n)+'.png' , img)
                        ...
                        if(cv.countNonZero(r[2]) > 2):
                            img = color.rgba2rgb(r[0])
                            # io.imsave(path+ '/masks/%s_%d_%d.png' % (
                            #         osp.splitext(osp.basename(slide_paths[i]))[0], r[3][0], r[3][1]), r[2])
                            # io.imsave(path+ '/masks_1/%s_%d_%d.png' % (
                            #         osp.splitext(osp.basename(slide_paths[i]))[0], r[3][0], r[3][1]), mask)
                            # io.imsave(path+'imgs/%s_%d_%d.png' % (
                            #         osp.splitext(osp.basename(slide_paths[i]))[0], r[3][0], r[3][1]), img)
                    n = n + 1
                    # print(r[2])
